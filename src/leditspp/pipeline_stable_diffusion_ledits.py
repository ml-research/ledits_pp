...
class StableDiffusionPipeline_LEDITS(DiffusionPipeline):
    r"""
    ...
    def encode_image(self, image_path, dtype=None):
        image = load_512(image_path,
                         sizes=(int(self.unet.sample_size * self.vae_scale_factor), int(self.unet.sample_size * self.vae_scale_factor*1.5)),
                         device=self.device,
                         dtype=dtype)
